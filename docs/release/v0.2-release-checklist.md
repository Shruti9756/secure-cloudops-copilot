...
- [ ] Run a clean-clone static verification from `codex/v02-identity-foundation` without copying `.env`, AWS credentials, access tokens, or browser session data.
- [ ] Open and merge the V0.2 release pull request into `main` after all CI checks pass.
- [ ] Create annotated Git tag `v0.2.0`.
- [ ] Publish GitHub Release `v0.2.0` with the release notes from `CHANGELOG.md`.
